...
/*************************************************************** FLASH_Start ***************************************************************/
/******************页配置(更换芯片必须修改这个配置)***********************/
#define Stockpile_Page_Size        0x800U        //扇区大小(默认1024字节)
...
typedef struct {
    ...
    uint32_t page_num;            //芯片实体页数量 这一段内存需要占据的内存页数大小
    //过程量
...
void
Stockpile_Flash_Data_Write_Data16(Stockpile_FLASH_Typedef *stockpile, uint16_t *data, uint32_t num);    //Flash_16位数据写入
void
Stockpile_Flash_Data_Write_Data32(Stockpile_FLASH_Typedef *stockpile, uint32_t *data, uint32_t num);    //Flash_32位数据写入
void
Stockpile_Flash_Data_Write_Data64(Stockpile_FLASH_Typedef *stockpile, uint64_t *data, uint32_t num);    //Flash_64位数据写入

void Stockpile_Flash_Data_QWrite_Data16(Stockpile_FLASH_Typedef *stockpile, uint16_t *data,
                                        uint32_t num);    //Flash_16位数据快速写入


uint16_t FLASH_ReadHalfWord(uint32_t address);

void Stockpile_Flash_Data_Read_Data(Stockpile_FLASH_Typedef *stockpile, void *data, uint32_t num);    //Flash位数据读取
